...
def readConfigFile(configFile):
    ...
    #print config["system"]["switch_cpus"][0]["clock"]
    F.close()
...
def prepareTemplate(outputFile):
    ...
    for child in root[0][0]:
        ...
        # if isinstance(temp, basestring) and "switch_cpus." in temp and temp.split('.')[0] == "stats":
        #     value = "(" + temp.replace("switch_cpus.", "cpu0.") + ")"
        #     for i in range(1, numCores):
        #         value = value + " + (" + temp.replace("switch_cpus.", "switch_cpus"+str(i)+".") +")"
        ...
        if child.attrib.get("name") == "core":
            ...
            for coreCounter in range(numCores):
                ...
                for coreChild in coreElem:
                    ...
                    # if isinstance(childValue, basestring) and "switch_cpus." in childValue and "stats" in childValue.split('.')[0]:
                    #     childValue = childValue.replace("switch_cpus." , "switch_cpus" + str(coreCounter)+ ".")

                    if isinstance(childValue, basestring) and "switch_cpus." in childValue and "config" in childValue.split('.')[0]:
                        childValue = childValue.replace("switch_cpus." , "switch_cpus." + str(coreCounter)+ ".")
                    ...
                    if len(list(coreChild)) is not 0:
                        for level2Child in coreChild:
                            ...
                            # if isinstance(level2ChildValue, basestring) and "switch_cpus." in level2ChildValue and "stats" in level2ChildValue.split('.')[0]:
                            #     level2ChildValue = level2ChildValue.replace("switch_cpus." , "switch_cpus" + str(coreCounter)+ ".")

                            if isinstance(level2ChildValue, basestring) and "switch_cpus." in level2ChildValue and "config" in level2ChildValue.split('.')[0]:
                                level2ChildValue = level2ChildValue.replace("switch_cpus." , "switch_cpus." + str(coreCounter)+ ".")
            ...
            if privateL2:
                ...
                for l2Counter in range(numL2):
                    ...
                    for l2Child in l2Elem:
                        ...
                        if isinstance(childValue, basestring) and "switch_cpus." in childValue and "stats" in childValue.split('.')[0]:
                            childValue = childValue.replace("switch_cpus." , "switch_cpus" + str(l2Counter)+ ".")
                        if isinstance(childValue, basestring) and "switch_cpus." in childValue and "config" in childValue.split('.')[0]:
                            childValue = childValue.replace("switch_cpus." , "switch_cpus." + str(l2Counter)+ ".")
                        if isinstance(childValue, basestring):
                ...
                for l2Child in child:
                    childValue = l2Child.attrib.get("value")
                    if isinstance(childValue, basestring) and "switch_cpus.l2cache." in childValue:
                        childValue = childValue.replace("switch_cpus.l2cache." , "l2.")
...
def getConfValue(confStr):
    ...
    for x in spltConf:
        ...
        if x.isdigit():
            ...
            #     #this is mostly for system.switch_cpus* as system.switch_cpus is an array
